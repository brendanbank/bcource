...
- DNS configured for `grafana.example.com` (or your configured domain)
...
- **Primary:** https://grafana.example.com
- **Alternative:** https://production-host (if configured)
...
- Custom domain Google Workspace accounts (e.g., @example.com, @example.com)
...
1. **Create Google OAuth Credentials:**
   ...
   - Configure:
     ...
     - **Authorized JavaScript origins:** `https://grafana.example.com`
     - **Authorized redirect URIs:** `https://grafana.example.com/login/google`
   - Click **Create**
...
4. **Test Login:**
   - Go to https://grafana.example.com
...
1. **Add to `.env` file:**
   ...
   GRAFANA_GOOGLE_ALLOWED_DOMAINS=gmail.com,google.com,example.com
...
1. **Admin deletes the existing user:**
   ...
   - Find the user `support@bcourse.nl`
...
- Ensure the redirect URI in Google Console matches exactly: `https://grafana.example.com/login/google`
- Check that your domain matches `grafana.example.com`

**"Access blocked" error:**
- Verify the authorized JavaScript origin is set to: `https://grafana.example.com`
...
- **Solution 1 (Recommended):** Log in with existing credentials first:
  1. Go to https://grafana.example.com
...
- **Solution 2 (If linking doesn't work):** The user account might need to be deleted and recreated:
  ...
  2. Find the user `support@bcourse.nl`
...
1. Log into Grafana at https://grafana.example.com
...
- Log into Grafana at https://grafana.example.com
...
1. **Check DNS:** Ensure `grafana.example.com` points to your server
2. **Check Traefik logs:**
   ```bash

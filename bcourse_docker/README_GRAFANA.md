...
Grafana is now hosted at: **https://grafana-upstream:3000** (HTTPS for encrypted connections)

Access via proxy: **https://grafana.example.com** (proxies to grafana-upstream:3000)

## Proxy Configuration

The `grafana.example.com` domain is configured using an nginx proxy container that forwards requests to the external Grafana server. This means the URL stays as `grafana.example.com` in the browser.
...
grafana-proxy:
  ...
  labels:
    - "traefik.http.routers.grafana-proxy.rule=Host(`grafana.example.com`)"
...
The nginx configuration (`grafana/nginx-proxy.conf`) proxies all requests to `https://grafana-upstream:3000` while:
- Preserving the original host header (`grafana.example.com`)
...
4. **Test direct connection to Grafana:**
   ```bash
   curl -I -k https://grafana-upstream:3000
   ```
...
3. **Verify Grafana accepts connections:**
   ...
   curl -k https://grafana-upstream:3000
   ```
...
1. **Test connectivity from Traefik container:**
   ...
   docker compose exec traefik wget -O- --no-check-certificate --timeout=10 https://grafana-upstream:3000
   ```
...
3. **Verify backend is accessible from host:**
   ...
   curl -k --max-time 10 https://grafana-upstream:3000
   ```

4. **Check Grafana health endpoint:**
   ```bash
   curl -k https://grafana-upstream:3000/api/health
   ```

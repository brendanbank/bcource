# Hetzner setup (bcourse.nur1.banzai.nl)

Host: `brendan@bcourse.nur1.banzai.nl`  
...
**Why it works on bcourse-app.fft1.banzai.nl but not bcourse.nur1.banzai.nl:** bcourse-app is **amd64** (x86_64); bcourse (Hetzner) is **arm64** (aarch64). The official `grafana/loki-docker-driver:latest` is amd64-native. On arm64 the plugin fails to create the socket. So the fix is architecture-specific (see below).
...
- `https://loki.banzai.nl:3100/loki/api/v1/push`
...
logging:
  driver: "loki"
  options:
    loki-url: "https://loki.banzai.nl:3100/loki/api/v1/push"
...
Ensure the Loki server is reachable from `bcourse.nur1.banzai.nl` (firewall and TLS).
...
| Loki URL | Adjust in compose or `.env` if Loki is not at `loki.banzai.nl` |
